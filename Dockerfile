...
ARG DOCKER_PHP_EXTENSION_INSTALLER=1.2.23
...
RUN chmod uga+x /usr/local/bin/install-php-extensions && chmod uga+x /usr/local/bin/wait-hosts && sync && \
    ...
    install-php-extensions gd calendar intl gmp exif opcache apcu memcached redis pcntl imagick ctype curl phar bz2 filter iconv ldap mcrypt bcmath xdebug imap mongodb pgsql oauth pdo_pgsql pdo_firebird pdo_odbc pdo_sqlsrv mysqli sqlsrv yaml json pdo simplexml xml tokenizer xmlwriter xmlreader pdo_mysql zip intl && \
